...
- ~~Ads as things in their own right~~ (asked for and built on 20 September 2026, not on the original list): an Ads tab that makes an ad directly and finds any ad by sponsor, and `POST /api/obs/setup`, which puts a deal's placements into OBS. The research note had called this "one confirmation instead of a paragraph of instructions"; it took the rule that the logger refuses a source showing another sponsor to make it safe.
