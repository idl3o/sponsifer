...
npm test           # 249 tests, including the calibration sweep
npm run typecheck
npm run lint       # includes the house rules: no function over 50 lines
python -m pytest   # 162 tests: the workspace server, receipts, SSH signatures, timestamps, seal and verify, the on-air log, its runner and the report
