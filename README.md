...
**Puts a sponsor on stream, and shows they were there.** This half is early. Each won deal has overlay addresses to add to OBS as browser sources — a corner emblem you place by hand over a stream frame, a lower third, a segment slate and a break card — drawn in your house style with the sponsor's own logo. Every one carries "Ad", asks OBS for no permissions, and never draws a price. While you stream, the on-air logger, started from the won deal in the app or as `sponsifer log` in a terminal, listens to OBS's own WebSocket and writes down, in wall-clock UTC, every second each placement was in the broadcast feed, checking OBS directly rather than trusting its events. Afterwards `sponsifer report` folds that log into intervals with offsets into the recording, signs it with your SSH key, and gives you the three files the sponsor keeps: the recording is the evidence, the report says where to look. The logger has been tested against a fake OBS, and OBS's own reporting against a real session: scene switches, visibility toggles and studio mode all produced matching events, with no disagreement from the direct checks. A live stream and a nested scene are still to be run. Started from the app, the logger has been run against OBS 32.2.2 itself, with no stream live, and against a stand-in OBS over a real socket for the password path. A dock inside OBS (Docks, Custom Browser Docks, `http://127.0.0.1:5180/dock.html`) shows which placements OBS has, which are in the program feed and which are on air this moment, and holds the logger's Start and Stop. Like the overlay it shows no price, because OBS's own window gets captured and shared. An Ads tab makes an ad directly for a sponsor you already have, without walking prospect and pricing first, and finds any ad by sponsor. One press puts a sponsor into OBS: the four sources are made if they are missing, hidden and at the canvas size, and the ones that exist are pointed at this deal, so your scenes are built once and the sponsor is what changes. It never removes, shows or hides anything, it will not run while the logger does, and the logger in turn refuses to begin while a source still shows another sponsor. That has been run against a stand-in OBS over a real socket, and its reads against OBS itself; the press that changes a real OBS is the streamer's to make.
