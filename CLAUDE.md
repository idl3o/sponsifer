...
- **The right deal at a dead address is a warning, not a refusal.** The logger runs, because the deal matches, and `Status.warnings` says the source loads from somewhere this server does not answer and may be drawing nothing. Found against a real OBS, where the refusal rule alone would have logged minutes of a blank overlay. The CLI logger cannot know a server's port and makes no such claim.
